import{s as c}from"./main-DT2TnJvn.js";const d=new URLSearchParams(window.location.search),l=Number(d.get("id"));async function g(){const{data:e,error:t}=await c.from("hymns").select(`
            *,
            authors(name),
            books(name),
            categories(name)
